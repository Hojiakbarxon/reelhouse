...
import { MovieCastManager } from "@/components/admin/MovieCastManager";
...
export function AdminMovieFormPage() {
  ...
  const listItem = useMemo(
    () => adminMovies?.movies.find((m) => m.id === id),
    [adminMovies, id],
  );
  const {
    data: movie,
  ...
  useEffect(() => {
    ...
    const categoryIds = categories
      .filter((c) => movie.categories.includes(c.name))
      .map((c) => c.id);
    reset({
      title: movie.title,
  ...
  if (
    isEditing &&
    (isLoadingAdminList || isLoadingCategories || (listItem && isLoadingMovie))
  ) {
